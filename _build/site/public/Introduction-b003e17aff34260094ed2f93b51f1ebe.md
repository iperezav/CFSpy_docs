...
The Lie derivative of :math:$h$ associated to the word $\eta$ is defined as

```{math}:label: eq:iter_lie
...
In applications, the infinite series in {eq}`eq-cfs` is truncated at some depth $N$.
